...
class CasaParser implements Parsable {
    ...
    public function getImages() {
        ...
        foreach ($picsData as $data) {
            $imagelink = "{$data->server}/{$resolution}{$data->uri}";
            //            $images[] = $imagelink;
            $imageName = "test1.jpg";
            saveImage($imagelink, $imageName);

            $overlayImage = data('images', 'casa_bottom');
            $coordinates = data('coordinates', 'casa_bottom');

            $options = ImagickOptions::arrayConstruct($imageName, $overlayImage, $coordinates);
            getNewImage($options);
            dd("", true);
            break;
        }
    ...
    //////////////////////////////////////// PRIVATEES ///////////////////////////////////////////////////////////
    private function overWriteImage($url) {
